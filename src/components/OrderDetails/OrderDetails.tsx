  ...
  return (
    <>
      {orderList.every((item) => item.count === 0) ? (
        <span>Order is empty! <br /> Please add some items!</span>
      ) : (
        orderList.map((item,index) => (
          item.count > 0 && (
            <div className="order-item" key={`order-${index}`}>
              ...
              <button className="button-delete" onClick={() => onClickButtonDelete(item.name)}>
                <svg className="ic-delete-trash" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" viewBox="0 0 26 26">
                  <path
